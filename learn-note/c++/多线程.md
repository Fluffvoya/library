...
wait_for等待指定时长，wait_until则等待到指定的时间点。返回值是一种枚举类型：
  ...
  - std::future_status::timeout 超时，数据还未准备好。
  - std::future_status::deferred 这个和std::async相关，表明无需wait，异步函数将在get时执行。
- `future::valid()`
...
### 7.3-memory_order
#### 7.3.1-引入
假如我们现在有两个线程分别叫T1和T2，T1执行任务A与任务B，T2执行任务C和任务D，假如ABCD四个任务都是原子操作，在CPU不对指令进行重排时会有六种情况：ABCD、ACBD、ACDB、CABD、CADB、CDAB。而如果A与B没有关联，CPU为了提高代码执行速度在不影响程序执行结果的情况下是有可能乱序执行，也就是有可能先执行B再执行A或者先执行D再执行C，而这些优化在多线程中有可能引发一些错误；例如：
```cpp
int i = 0;
...
```
这段代码中的t2会等待a被设置成true后立刻判断i是否等于42，常理上a会变成true前i已经为42，但为了优化，编译器和cpu可能会进行指令重排，或内存可见性原因造成的指令重排表象，即a可能先变成true了，这个结果是未知的，因此需要内存顺序来保证结果的可预见性以及正确性。
#### 7.3.2-宽松次序
memory_order_relaxed只保证原子操作，不对其他任何顺序进行保证，即可以随意发生重排，通常用于计数器,例如：
```cpp
...
即`release`和`acquire`
##### release
一般用于`store`，它保证了该条命令前的所有内存操作（包括非原子的）不会被重排到本条命令后
##### acquire
一般用于`load`，它保证了该条命令后的所有的内存操作（包括非原子的）不会被重排的本条指令前
#### 7.3.4-RMW
即Read Modify Write，读改写，适用于`exchange`族及`fetch_add`这些，使用`acq_rel`，保证命令前的操作不会排到后面，命令后的操作不会排到前面
#### 7.3.5-顺序一致性
指的就是memory_order_seq_cst，对于acquire和release来说，它们不会要求全局顺序一致，即不同线程观测可能会产生不同的执行顺序，而seq_cst保证了全局一致性，其他线程观测时多个原子变量的操作顺序是一致的，不会出现IRIW问题